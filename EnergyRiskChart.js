...
export class EnergyRiskChart {
  ...
  _fuelDataForStep(step) {
    ...
    this.scenarioData.forEach((d) => {
      ...
      if (!map.has(d.fuel))
        map.set(d.fuel, { fuel: d.fuel, value: 0, prevValue: 0 });
  ...
  _precompute() {
    for (let step = 1; step <= this.maxStep; step++) {
      ...
      const totalActualArea = cells.reduce(
        (s, c) => s + (c.polygon ? polyArea(c.polygon) : 0),
        0,
      );
      const totalDots = Math.max(
        MIN_DOTS * cells.length,
      ...
      cells.forEach((cell) => {
        ...
        cell.particles = cell.polygon
          ? sampleInPolygon(cell.polygon, totalCount)
          : [];
  ...
  _setupLegend() {
    ...
    fuels.forEach((fuel, i) => {
      const row = this.legendGroup
        .append("g")
        .attr("transform", `translate(0,${startY + i * ROW_H})`);
      row
        .append("rect")
        .attr("x", dotX)
        .attr("y", 0)
  ...
  updateStep(step) {
    ...
    const borderSel = this.bordersGroup.selectAll("path.fuel-border").data(
      cells.filter((c) => c.polygon && c.polygon.length >= 3),
      (c) => c.fuel,
    );
    ...
    borderSel
      ...
      .on("mouseover", (event, c) => {
        ...
        this.tooltip
          .style("opacity", 1)
          .html(
            `<strong>${c.fuel}</strong><br/>Risk index: ${c.value.toFixed(2)}`,
          );
      })
      .on("mousemove", (event) => {
        this.tooltip
    ...
    cells.forEach((cell) => {
      ...
      const particleData = particles.slice(0, totalCount).map((pos, idx) => {
        ...
        return {
          ...
          isNew,
          fuel,
          fuelValue: cell.value,
        };
      });

      const sel = g
        .selectAll("circle.risk-particle")
